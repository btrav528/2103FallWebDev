...
class Card {
	...
	static public function Save($row) {
		...
		if ($row['Id'] != null) {
			$sql = "UPDATE 2013Fall_CreditCard " . "SET NameOnCard='" . $row2['NameOnCard'] . "', CardNumber='" . $row2["CardNumber"] . "', SecurityCode='" . $row2["SecurityCode"] . "', ExpDate='" . $row2["ExpDate"] . "', Nickname='" . $row2["Nickname"] . "' WHERE Id='" . $row2['Id'] . "';";
		} else {
			$sql = "Insert Into 2013Fall_CreditCard (NameOnCard, CardNumber, SecurityCode, ExpDate, Nickname,BillingAddress_Id, 2013Fall_User_Id ) Values ('" . $row2["NameOnCard"] . "', '" . $row2["CardNumber"] . "', '" . $row2["SecurityCode"] . "', '" . $row2["ExpDate"] . "', '" . $row2["Nickname"] . "', '1', '1') ";
		}
		echo $sql;
		$conn -> query($sql);
		//echo $sql;
