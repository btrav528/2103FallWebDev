...
class Orders {
	static public function FinalSale($row, $orderNumber) {
		...
		$conn = GetConnection();
		$sql = " Insert Into 2013Fall_Orders (`2013Fall_User_id`, , `OrderNumber`, `DateOrdered`, `DateShipped`)" . " Values ('$row[Users_id]','$orderNumber', '$orderDate', '$shipDate') ";
	...
	static public function FinalSaleDe($row, $orderNumber) {
		$conn = GetConnection();
		$sql = " Insert Into 2013Fall_Order (`OrderTotal`, `OrderNumber`) " . " Values ('$row[Price]', '$orderNumber')";
	...
	static public function Save($row) {
		...
		if ($row['Id'] != null) {
			$sql = "UPDATE 2013Fall_Orders " . "SET OrderNumber='" . $row2['OrderNumber'] . "', DateOrdered='" . $row2["DateOrdered"] . "', DateShipped='" . $row2["DateShipped"] . "', OrderTotal='" . $row2["OrderTotal"] . "', 2013Fall_User_Id='" . $row2['2013Fall_User_Id'] . "' WHERE Id='" . $row2['Id'] . "';";
		} else {
			$sql = "Insert Into 2013Fall_Orders (OrderNumber, DateOrdered, DateShipped, OrderTotal, 2013Fall_User_Id) Values ('" . $row2["OrderNumber"] . "', '" . $row2["DateOrdered"] . "', '" . $row2["DateShipped"] . "', '" . $row2["OrderTotal"] . "', '" . $row2["2013Fall_User_Id"] . "') ";
		}
		echo $sql;
		$conn -> query($sql);
		//echo $sql;
